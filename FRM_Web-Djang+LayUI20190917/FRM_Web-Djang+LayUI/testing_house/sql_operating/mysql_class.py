...
class Mysql_client(Mysql_base):
    ...
    class Mysql_client_FRM(Mysql_base):
        ...
        def get_purchase_apply_table(self):
            self.select(self, )
...
class Mysql_client_FRM(Mysql_base):
    ...
    def get_select(self, table, fields, condition, order_by=None, limit=None,
               desc=False):
        try:
            print(">>>>>>>>>>>>>>>>>>>>>>",table,fields, condition, order_by,limit,desc)
            purchase_id  = self.select(self, table,fields, condition, order_by, desc, limit )
            print('查询请购信息表id===:')
            return   purchase_id
        except Exception as e:
            print('没有id请从1开始',e )
