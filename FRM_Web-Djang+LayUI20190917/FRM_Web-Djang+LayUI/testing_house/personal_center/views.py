...
#   TODO  任务列表页  start  function
def set_startjobs_info(request):
...
def get_isa_info(job_no=None):
    ...
    try:
        ...
        once_exe_row = mysql_client.get_rpa_exe(filed=job_no);
...
def update_sql(user_name):
    try:
        ...
        isa_job_no = Job_list_summary.objects.filter(~Q(isdelete=1) & Q(user_name_id = user_name) &
                                                     ~Q(isa_job_no=None) & Q(job_status='1110') | Q(job_status='1111'))
